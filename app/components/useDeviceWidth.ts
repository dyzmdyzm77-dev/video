...
export function useDeviceWide(scope?: 0 | 1 | 2): boolean {
  ...
  useEffect(() => {
    const read = () => setWide(readDeviceWide(s));
    read();
    const evts = [
      "devicechange",
      "devicerange",
...
export function useDeviceRatio(scope?: 0 | 1 | 2) {
  ...
  const [r, setR] = useState(
    () => readDeviceWidth(undefined, s) / readDeviceHeight(s),
  );
  useEffect(() => {
    const read = () => setR(readDeviceWidth(undefined, s) / readDeviceHeight(s));
    read();
...
export function useDeviceWidth(scope?: 0 | 1 | 2) {
  ...
  const [w, setW] = useState(() => readDeviceWidth(undefined, s));
  useEffect(() => {
    const read = () => setW(readDeviceWidth(undefined, s));
    read();
