...
// 화면에서 달라진다. 그때마다 안을 따로 파지 않고, 좌측 데스크톱
// 패널(DesktopVariantNav)의 토글 하나로 두 상태를 다 볼 수 있게 한다.
//
//   nvr (기본) — 로컬 녹화기. 움직임 감지 카드에 썸네일이 없다.
//   cloud      — 클라우드. 썸네일이 있다.
//
// 지금 이 값이 가르는 건 감지 썸네일 하나뿐이다(eventThumbs.ts). 원래 그 자리엔
// '감지 썸네일' 체크박스가 따로 있었는데, 체크 여부가 곧 클라우드냐 NVR 이냐라는
// 게 확인돼서(2026-08-21 사용자) 이름을 제대로 붙여 여기로 합쳤다. 앞으로 저장
// 방식이 다른 것까지 가르게 되면 그 분기도 이 값을 읽는다.
//
...
// 유일한 진입점이다. 합치기 전의 ?thumbs=0/1 도 같은 자리에서 계속 받는다 —
// 이미 돌아다니는 링크가 있다.
// ============================================================================
