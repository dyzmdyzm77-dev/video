...
- **단일 영상은 기본이 정확히 16:9.** `globals.css` 의 `.single-video-area` / `.single-video-box`.
  ...
  **예외 하나 — 카메라 목록이 가로 스크롤일 때는 16:9 를 넘겨 세로로 늘어난다.**
  목록 스트립을 96 으로 못 박고 남는 세로를 영상이 다 가져가기 때문(620×780 에서
...
- **가로 1줄일 때 목록 영역 높이 = 움직임 감지 탭 높이(`MOTION_MIN_H` 96).**
  ...
  **영역 여백은 위 12 · 아래 0 이다**(사용자 지정 2026-08-31: "12px 흰띠 없애").
  아래 여백은 스크롤러 밖이라 목록을 끝까지 내려도 하단 탭 위에 흰 띠로 남았다.
  이 여백과 못 박는 높이는 항상 같이 움직인다 — 여백만 지우면 타일이 12 커진다.
